 ...
 * GeoNames gazetteer (coordinates + H3 cells) in batches until no progress.
 *
...
async function main() {
  ...
  const [total, linked, withH3] = await Promise.all([
    prisma.signalLocation.count(),
    prisma.signalLocation.count({ where: { locationId: { not: null } } }),
    prisma.signalLocation.count({ where: { h3Cell: { not: null } } }),
  ]);
  console.log(
    `[BACKFILL] done in ${Math.round((Date.now() - t0) / 1000)}s: ` +
      `${linked}/${total} linked (${((linked / Math.max(total, 1)) * 100).toFixed(1)}%), ${withH3} h3-tagged`,
  );
}
